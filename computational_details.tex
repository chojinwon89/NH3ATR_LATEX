...
The thermodynamically stable surface at any oxygen potential is the termination of
lowest $\gamma$; following this minimum across $\Delta\mu_{\mathrm O}$ defines the
\emph{convex hull}, the lower envelope of all the lines in
Figure~\ref{fig:bulkref}. On the oxygen-rich (right) side the hull follows the
steeply sloped, oxygen-covered \textit{o}-RuO$_2$(110) terminations; on the
oxygen-poor (left) side it follows the flat stoichiometric \textit{s}-RuO$_2$(110)
0v surface. The dotted vertical line marks the hull vertex where the two branches
meet---the \textit{o}$\rightarrow$\textit{s} crossover---at
$\Delta\mu_{\mathrm O}=-1.02$, $-1.17$, $-1.36$ and $-1.58$~eV for 298, 600, 900
and 1200~K, respectively. Whenever the local oxygen potential lies to the right of
this vertex the oxidised \textit{o}-RuO$_2$(110) surface is stable; to the left,
the surface has released its on-top oxygens to become the reduced
\textit{s}-RuO$_2$(110) termination. The closer an operating point sits to the
vertex, the smaller the free-energy margin (a few meV\,\AA$^{-2}$) between the two
terminations, so near the crossover the oxidised and reduced surfaces are
effectively degenerate within DFT accuracy.
...
\begin{figure*}[htbp]
  ...
  convex-hull \textit{o}$\rightarrow$\textit{s} transition (\textit{o}-RuO$_2$ 3v
  $\rightarrow$ \textit{s}-RuO$_2$ 0v). Red and green dashed verticals locate the
...
\begin{figure*}[htbp]
  ...
  is flat. The dotted line marks the convex-hull
  \textit{o}$\rightarrow$\textit{s} crossover (the vertex of the lower envelope),
  which is identical to that of Figure~\ref{fig:unified}. Shaded bands are
  thermodynamically
