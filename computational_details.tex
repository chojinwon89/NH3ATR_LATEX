...
negative $\Delta\mu_{\mathrm O}$ denotes more reducing conditions. Replacing the
thermal term $\tfrac{1}{2}[G_{\mathrm{O_2}}(T)-G_{\mathrm{O_2}}(0\,\mathrm{K})]$
with experimental JANAF values shifts every threshold by $\le 0.05$~eV per O atom
and leaves the stability ordering unchanged.
...
threshold unchanged.
In Figure~7 each phase is referenced to its own 0v surface; in the unified
construction all terminations share the single \textit{o}-RuO$_2$(110) 0v
reference, using the fact that \textit{s}-RuO$_2$(110) 0v is
\textit{o}-RuO$_2$(110) with its six on-top oxygens removed
(Ru$_{36}$O$_{78} \rightarrow$ Ru$_{36}$O$_{72}$, $n_{\mathrm{vac}}=6$).
...
\begin{equation}
  ...
  \label{eq:down}
\end{equation}
evaluated at $p_{\mathrm{H_2O}}/p_{\mathrm{H_2}}=10$. The upper axis of Figure~7
converts $\Delta\mu_{\mathrm O}$ into an equivalent O$_2$ partial pressure by
...
numerically identical to that of Figure~7, confirming that the surface phase
...
\begin{figure*}[htbp]
  ...
  is flat. The dotted line marks the \textit{o}$\rightarrow$\textit{s} crossover,
  which is identical to that of Figure~7. Shaded bands are thermodynamically
  ...
  (decomposition to metallic Ru, Eq.~\eqref{eq:opoor}). All panels share a
  common $\gamma$ axis.}
  \label{fig:bulkref}
\end{figure*}
